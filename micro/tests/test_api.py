...
def test_object_creation(client, auth):
    ...
    responses.add(
        responses.POST,
        "https://applifting-python-excercise-ms.herokuapp.com/api/v1/products/register",  # noqa: E501
        status=201,
...
def test_unauthorized_creation(client):
    ...
    payload = {
        "name": "Pistole W0D4",
        "description": "Hračková pistole na vodu."
    }
...
def test_update_product(client, auth):
    ...
    payload = {
        "id": 1,
        "name": "Baťa cvičky",
        "description": "Retro sportovní boty světoznáme značky.",
    }
...
def test_unauthorized_update(client):
    ...
    payload = {
        "id": 1,
        "name": "Pruhy Zeebra",
        "description": "Nalepovací maškarní zebra-pruhy.",
    }
...
def test_edit_nonexistent(client, auth):
    ...
    payload = {
        "id": 4, "name": "Ilumix",
        "description": "Stylova stolni lampa."
    }
    r = client.put(f"{ROUTE}/update", headers={"Bearer": auth}, json=payload)
...
def test_get_product_details(client, auth):
    ...
    assert o1_payload in offers and o2_payload in offers
